...
public class PgnParser
{
  ...
  */
    ...
    public static IMove FromSan(Board board, string? san)
    {
        ...
        foreach (var moveToTest in board.CurrentPlayer.Moves)
        {
            ...
            else if (FileNames.Contains(san?[0].ToString()!))
            {
                ...
                if (san != null && FileNames.IndexOf(san[0]) == FileIndex(fromCoordinate))
                {
                    // Is promotion
                    if (san.Contains("="))
                    {
            ...
            else
            {
                ...
                if (FileNames.IndexOf(targetFile) == FileIndex(toCoordinate))
                    ...
                    if (RankNames.IndexOf(targetRank) == RankIndex(toCoordinate))
                    {
                        ...
                        if (ResolveAmbiguity(san, fromCoordinate)) continue;
    ...
    /// <summary>
    ///     Resolves ambiguity from same piece types being able to move to the same square.
    /// </summary>
    /// <param name="san">The string to parse.</param>
    /// <param name="fromCoordinate">The initial coordinate of the piece.</param>
    /// <returns></returns>
    private static bool ResolveAmbiguity(string? san, int fromCoordinate)
    {
        if (san is { Length: 4 })
        {
            ...
            {
                if (FileNames.IndexOf(disambiguationChar) != FileIndex(fromCoordinate)) return true;
            }
            // Is rank disambiguation
            else
            {
                if (RankNames.IndexOf(disambiguationChar) != RankIndex(fromCoordinate)) return true;
            }
        }
