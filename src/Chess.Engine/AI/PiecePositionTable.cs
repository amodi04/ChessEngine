...
namespace Engine.AI
{
    ...
    public static class PiecePositionTable
    {
        ...
        public static int Read(int[] table, int tileCoordinate, bool isWhite)
        {
            ...
            if (isWhite)
            {
	            // Get the file and rank indexes
                int file = BoardUtilities.FileIndex(tileCoordinate);
                int rank = BoardUtilities.RankIndex(tileCoordinate);

                // Calculate the new tile coordinate based on rank and file
                // This is because the piece position tables are stored in a top left to bottom right format
                // Rather than the boards which are bottom left to top right
                // This converts the tile coordinate to white's perspective
                rank = 7 - rank;
            ...
            // Return the value at the index of the chosen table
